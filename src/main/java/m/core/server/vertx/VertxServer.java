...
public class VertxServer extends BaseServer {
    ...
    private @Nullable m.core.http.Session formatSession(@Nullable Session vSession) {
        // Could be null without a session handler
        if (vSession == null) { // TODO Check isDestroyed and isEmpty
            return null;
        }
        m.core.http.Session session = new m.core.http.Session().setId(vSession.id())
    ...
    private void updateSession(@Nullable Session vSession, @Nullable m.core.http.Session session) {
